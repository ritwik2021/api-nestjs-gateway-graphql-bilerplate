...
export function customInputValidation(request, response, next) {
  try {
    ...
    if (error && Object.keys(error).length && error['errorStatus']) rejectRequest(error);
...
function findInjection(input_params: any) {
  ...
  for (const key in input_params) {
    if (input_params[key] || typeof input_params[key] === 'boolean') {
      let keyValue = typeof input_params[key] === 'string' ? input_params[key].replace(/%20/g, ' ') : input_params[key];

      if (typeof input_params[key] === 'string') {
        keyValue = sanitizeUrl(keyValue); /* sanitize this if its invalid url */
        const noHtmlString = stripeHtmlScriptTags(keyValue); /* stripping out html and script tags */
        input_params[key] = noHtmlString;
        isInjectionFound = applyInjectionRule(noHtmlString); /* applying the injection rules xss and sql */
      }
      error = {
        requestKey: key,
